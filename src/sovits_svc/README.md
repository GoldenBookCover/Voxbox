...
### 底模的版权链接：https://github.com/PlayVoice/so-vits-svc-5.0/issues/112 使用vctk-english & opensinger-chinese开源数据集训练，无版权问题
...
1. 导出推理模型：文本编码器，Flow网络，Decoder网络；判别器和后验编码器等只在训练中使用。刚训练好的是pt格式，导出是pth格式  
   ```
   python svc_export.py --config configs/base.yaml --checkpoint_path chkpt/sovits5.0/sovits5.0_0100.pt
   ```
2. 推理  
